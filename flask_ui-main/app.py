...
def index():
    ...
    stats = {
        ...
        'total': 1832,
        'rev_table' : playstore.pivot_table(index = ['Category','App'],
                                  values = ['Reviews','Rating'],
                                  aggfunc = 'sum'
                                  ).sort_values(by = 'Reviews', ascending=False).head(10).reset_index().reindex(['Category','App','Reviews','Rating'],axis=1).to_html(classes=['table thead-light table-striped table-bordered table-hover table-sm'])
    }
    ...
    X = [1832, 959, 827, 420, 395]
    Y = ['FAMILY','GAME','TOOLS','BUSINESS','MEDICAL']
    ...
    Type = playstore.groupby('Content Rating').agg({
    'App' : 'count'
    }).rename({'App':'Total'}, axis=1).sort_values('Total',ascending=False).head()
    y = np.array([7903, 1036, 393, 322, 3])
    mylabels = ["Everyone", "Teen", "Mature 17+", "Everyone 10+", "Adults only 18+"]
